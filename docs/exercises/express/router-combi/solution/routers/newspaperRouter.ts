...
export function newspaperRouter() {
    const router = express.Router();

    router.get("/search", (req, res) => {
        if (typeof req.query.q === "string") {
            if (req.query.q === "") {
                res.render("newspaper/search", { articles: [], topics, query: "" });
                return;
            }
            ...
            res.render("newspaper/search", { articles: filteredArticles, topics, query: query });
        } else {
            res.render("newspaper/search", { articles: [], topics, query: "" });
        }
    });
    ...
    router.get("/", (req, res) => {
        res.render("newspaper/articles", { articles: articles, topics });
    });
    ...
    router.get("/:topic", (req, res) => {
        ...
        res.render("newspaper/articles", { articles: filteredArticles, topics });
    });
    
    router.use((req, res) => {
        res.status(404).render("newspaper/page_404", { topics: topics });
    });
